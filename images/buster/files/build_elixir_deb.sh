...
if [[ -z "$1" ]]; then
  echo "usage: ./build_elixir_deb.sh <release> <github_filename>"
  exit 1;
fi

# 1. Download the elixir release
SE_ELIXIR_RELEASE=$1
SE_GITHUB_FILENAME=$2
...
if [[ ! -f "$SE_ELIXIR_RELEASE_FILE" ]]; then
  wget -O $SE_ELIXIR_RELEASE_FILE "https://github.com/elixir-lang/elixir/releases/download/v$SE_ELIXIR_RELEASE/$SE_GITHUB_FILENAME.zip"
fi
